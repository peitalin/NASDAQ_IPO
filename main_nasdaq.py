...
def test_cik(cik):
    ...
    print(cik, finaljson[cik]['Company Overview']['Company Name'])
...
if __name__=='__main__':
    ...
    # rawjson = json.loads(open('rawjson.txt').read())
    ######## NEW BAD INDUSTRIES #############
    ...
    # with open('rawjson.txt', 'w') as f:
    #     f.write(json.dumps(rawjson, indent=4, sort_keys=True))
...
def gtrends_names():
    # gtrends_ciklist = [(cik,
    #                     val['Company Overview']['Company Name'],
    #                     re.findall(r"\d{0,2}/\d{0,2}/\d+", val['Company Overview']['Status'])[0])
    #                     for cik,val in rawjson.items()]
    # gdat = pd.DataFrame(gtrends_ciklist, columns=['cik', 'coname', 'date'])
    # gdat.set_index('cik', inplace=True)
    # gdat['date'] = [parse_date(s).strftime('%m-%d-%Y') for s in gdat['date']]
